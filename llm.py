...
- Если пользователь просит показать задачи, проекты или заметки — ОБЯЗАТЕЛЬНО вызывай соответствующий инструмент (get_tasks, get_projects, search_notes), не пытайся вспомнить их сам.
- Инструменты для создания используй только по явному запросу ("создай", "сохрани", "запиши")
